...
download.file("https://zenodo.org/records/6580069/files/10X_Visium_ACH005.tar.gz?download=1",
    destfile = "data/10X_Visium_ACH005.tar.gz", method = "curl")
untar("data/10X_Visium_ACH005.tar.gz")
```
...
```{r}
seurat <- readRDS("data/ACH005/ACH005.rds")
...
```{r message=FALSE, warning=FALSE}
run_misty(com_path_act_views, "data/result/comp_path_act")
misty_results_com_path_act <- collect_results("data/result/comp_path_act/")
```
...
```{r message=FALSE, warning=FALSE}
run_misty(com_path_act_views, "data/result/comp_path_act_linear", model.function = linear_model, bypass.intra = TRUE)
misty_results_com_path_act_linear <- collect_results("data/result/comp_path_act_linear")
```
...
# Run Misty
run_misty(comp_TF_path_views, "data/result/comp_TF_path", model.function = linear_model, bypass.intra = TRUE)
misty_results_comp_TF_pathway <- collect_results("data/result/comp_TF_path")
```
...
download.file("https://raw.githubusercontent.com/saezlab/liana-py/main/liana/resource/omni_resource.csv", 
    destfile = "data/omni_resource.csv", method = "curl")

# Ligand Receptor Resource
omni_resource <- read_csv("data/omni_resource.csv")%>% 
...
run_misty(lig_recep_view, "data/result/lig_recep", bypass.intra = TRUE)
misty_results_lig_recep <- collect_results("data/result/lig_recep")
```
